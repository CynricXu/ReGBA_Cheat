...
uint32_t PerGameVibrateLevel = 0;
uint32_t VibrateLevel = 0;
...
enum OpenDingux_Buttons MenuKeys[9] = {
...
enum GUI_Action MenuKeysToGUI[9] = {
	GUI_ACTION_ENTER,
	GUI_ACTION_LEAVE,
	GUI_ACTION_DOWN,
	GUI_ACTION_UP,
		...
		switch (ev.type)
		{
			...
			case SDL_MOUSEBUTTONDOWN:
				if(ev.button.button == SDL_BUTTON_LEFT){
					FutureButtons |= 1 << 19;
					if ((LastButtons & (1 << 19)) == (CurButtons & (1 << 19)))
						CurButtons |= 1 << 19;
					break;
				}else{
					FutureButtons |= 1 << 20;
					if ((LastButtons & (1 << 20)) == (CurButtons & (1 << 20)))
						CurButtons |= 1 << 20;
					break;
				}
				break;
...
void ProcessSpecialKeys()
{
	...
	{
		SetMenuResolution();
		if(ShowWarningScreen("读取快速存档吗？这将会失去当前状态\n[A] = 是  其他键 = 否")){
			switch (load_state(0))
			{
				case 1:
					if (errno == 2)
						ShowErrorScreen("读取存档 #1 错误：未找到存档文件");
					else if (errno != 0)
						ShowErrorScreen("读取存档 #1 错误：\n%s", strerror(errno));
					else
						ShowErrorScreen("读取存档 #1 错误：不完整的文件");
					break;

				case 2:
					ShowErrorScreen("读取存档 #1 错误：文件格式无效");
					break;
			}
		}
		ScaleModeUnapplied();
	...
	{
		if(ShowWarningScreen("保存快速存档吗？这会覆盖当前的保存\n[A] = 是  其他键 = 否")){
			void* Screenshot = copy_screen();
			SetMenuResolution();
			if (Screenshot == NULL)
				ShowErrorScreen("生成快照保存在 #1 失败: 内存分配错误");
			else
			{
...
void Shake(){
	uint32_t level;
	level = ResolveSetting(VibrateLevel, PerGameVibrateLevel);
	switch (level){
		case 0:
			break;
		case 1:
			Shake_Play(device, id_shake_low);
			break;
		case 2:
			Shake_Play(device, id_shake_medium);
			break;
		case 3:
			Shake_Play(device, id_shake_strong);
			break;
		default:
			break;
	}
