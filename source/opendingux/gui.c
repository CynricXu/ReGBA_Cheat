...
static struct MenuEntry PerGameDisplayMenu_ScaleMode = {
	...
	.ChoiceCount = 9, .Choices = { { "未覆盖", "" }, { "Aspect, fast", "aspect" }, { "Full, fast", "fullscreen" }, { "Aspect, bilinear", "aspect_bilinear" }, { "Full, bilinear", "fullscreen_bilinear" }, { "Aspect, sub-pixel", "aspect_subpixel" }, { "Full, sub-pixel", "fullscreen_subpixel" }, { "None", "original" }, { "Hardware", "hardware" } }
};
static struct MenuEntry DisplayMenu_ScaleMode = {
	ENTRY_OPTION("image_size", "图像设置", &ScaleMode),
	.ChoiceCount = 8, .Choices = { { "拉伸, 快速渲染", "aspect" }, { "全屏, 快速渲染", "fullscreen" }, { "拉伸, 双线性", "aspect_bilinear" }, { "全屏, 双线性", "fullscreen_bilinear" }, { "拉伸, 子像素卷积", "aspect_subpixel" }, { "全屏, 子像素卷积", "fullscreen_subpixel" }, { "经典", "original" }, { "硬件渲染", "hardware" } }
};
#endif
...
static struct Menu PerGameMainMenu = {
	.Parent = NULL, .Title = "ReGBA 中文版 V7.0\nby Cynric",
	MENU_PER_GAME,
...
#if SCREEN_HEIGHT >= 240
	.Entries = { &MainMenu_Display, &MainMenu_Input, &MainMenu_Hotkey, &MainMenu_Cheats, &MainMenu_SavedStates, &Strut, &Strut, &MainMenu_Debug, &Strut, &MainMenu_Reset, &MainMenu_Return, &MainMenu_Exit, NULL }
#else
