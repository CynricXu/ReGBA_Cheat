...
int is_new_cheat(char *line){
	if(strstr(line,"direct") == NULL&&strstr(line,"gameshark") == NULL&&strstr(line,"PAR") == NULL){
		return 0;
	}else{
		return 1;
	}
...
void add_cheats(char *cheats_filename)
{
  ...
  if (DEFAULT_CHEAT_DIR != NULL) {
    sprintf(cheat_path, "%s/%s.cht", DEFAULT_CHEAT_DIR, cheats_filename);
  }
  else
  {
...
//printf("cheat%d:%s\n", g_num_cheats, game_config.cheats_flag[g_num_cheats].cheat_name);
        ...
        num_cheat_lines = 0;
			...
			while(fgets(current_line, 256, cheats_file))
			{
			  if(strlen(current_line) < 3 || is_new_cheat(current_line))
				break;
  ...
  for(i = 0; i < g_num_cheats; i++)
  {
    if(game_config.cheats_flag[i].cheat_active == 1)
    {
      switch(game_config.cheats_flag[i].cheat_variant)
      {
		...
		/*
		FILE *logfile;	
		logfile = fopen("/tmp/regbalog.txt", "a+");
		if(logfile)
		{
			fputs(game_config.cheats_flag[i].cheat_name, logfile);
			char s[20];
			sprintf(s, "%d", game_config.cheats_flag[i].cheat_active);
			fputs(s, logfile);
			fputs("\n", logfile);
		}
		fclose(logfile); 
